...
public class AdminView extends AppCompatActivity {
    ...
    public void setDataForSelectDate(String fromDatePicker) {
        ...
        binding.lackData.setText(getString(R.string.lack_data_for)  +materialDatePicker.getHeaderText());
        ...
        if (!timeModelArrayListForAdmin.isEmpty()) {
            for (int j = 0; j < timeModelArrayListForAdmin.size(); j++) {
                for (int k = 0; k < arrayListOfSelectedDate.size(); k++) {
                    if (formatDateWithMonthAndYear(timeModelArrayListForAdmin.get(j).getTimeAdded().toDate()).equals(arrayListOfSelectedDate.get(k))) {
                        ...
                        //Log.d("TMFD Method", arrayListFromInsideSelectDateMethod.get(j).getTimeBegin());
                        break;
                    }
                }
            }
            ...
            if(brandNewArrayList.isEmpty()) {
              /*  for (TimeModelForDisplay element : brandNewArrayList) {
    ...
    private ArrayList<TimeModelForDisplay> summingTimeFromDatePicker(LinkedList<TimeModel> arrayListFromInsideSelectDateMethod) {
        ...
        if(arrayListFromInsideSelectDateMethod.size()==1)
        {
            ...
            timeModel.setId(arrayListFromInsideSelectDateMethod.get(0).getId());
            timeModel.setTimeOverallInLongLefToSettle(0);
            timeModel.setWithdrawnMoney(0);
            timeModelsForDisplay.add(timeModel);
        }
        else if(arrayListFromInsideSelectDateMethod.size()>1) {
